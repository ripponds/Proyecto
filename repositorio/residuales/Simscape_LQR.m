...
%% ── 7. RESTRICCIÓN CINEMÁTICA DURA: x = r * phi_R ───────────────────────
%  Joint_phi_R.RConn(2) = phi_R (posición angular acumulada)
%  x_cmd = r * phi_R  →  Joint_x (InputMotion)
%  Esto impone algebraicamente x = r*phi_R sin fuerzas artificiales.

add_block('nesl_utility/PS-Simulink Converter',[modelName '/PS2SL_phi'],'Position',[200 680 310 710]);
pH_PS_phi = get_param([modelName '/PS2SL_phi'],'PortHandles');
add_line(modelName, pH_JpR.RConn(2), pH_PS_phi.LConn(1), 'autorouting','on');

add_block('simulink/Math Operations/Gain',[modelName '/Gain_r'],'Position',[360 680 440 710]);
set_param([modelName '/Gain_r'],'Gain',num2str(r));
pH_Gr = get_param([modelName '/Gain_r'],'PortHandles');
add_line(modelName, pH_PS_phi.Outport(1), pH_Gr.Inport(1), 'autorouting','on');

add_block('nesl_utility/Simulink-PS Converter',[modelName '/SL2PS_xm'],'Position',[480 680 570 710]);
pH_SL_xm = get_param([modelName '/SL2PS_xm'],'PortHandles');
add_line(modelName, pH_Gr.Outport(1), pH_SL_xm.Inport(1), 'autorouting','on');
add_line(modelName,'SL2PS_xm/RConn1','Joint_x/LConn2');

fprintf('[Kinematic] Restricción x = r*phi_R → Joint_x (InputMotion) OK\n');
