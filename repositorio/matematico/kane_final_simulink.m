%% ========================================================================
%  validacion_robusta_lqr.m — Planta Lineal vs No Lineal con LQR Ideal
%  MATLAB 2025b | Generación Automática de Simulink
% =========================================================================

clear; clc; close all;
warning('off', 'Simulink:Engine:MdlFileShadowing');

%% S0 a S11 — TU CÓDIGO OPTIMIZADO INTACTO
% (Se ejecuta silenciosamente para extraer A_num y B_num)
fprintf('1. Derivando modelo matemático...\n');
...
syms V_R V_L T_d real % <-- Añadimos T_d (Torque de perturbación)
...
A_num(abs(A_num) < 1e-10) = 0; B_num(abs(B_num) < 1e-10) = 0; Bd_num(abs(Bd_num) < 1e-10) = 0;

%% S12 — DISEÑO DEL CONTROLADOR LQR ESTRICTO
fprintf('2. Diseñando LQR (Enfoque en Cabeceo y Velocidad)...\n');
% Pesos: [Theta, dTheta, x, dx, alpha, dalpha]
% x y alpha tienen 1e-4 para evitar singularidad matemática. Físicamente es 0.
Q = diag([2000, 100, 1e-4, 50, 1e-4, 1e-4]); 
R = diag([1, 1]); % Castigo estándar a la energía de los motores

K = lqr(A_num, B_num, Q, R);
B_aug = [B_num, Bd_num]; % Matriz B aumentada para aceptar T_d en la simulación lineal
C_num = eye(6); D_aug = zeros(6, 3);
...
% --- Bloque de Perturbación (Torque T_d) ---
...
% Planta NL con 3 puertos separados: fcn(X, U, Td)
% X llega del integrador (puerto 1) — sin persistent, estado correcto en todo instante
add_block('simulink/User-Defined Functions/MATLAB Function', [modelName '/Planta_NoLineal'], 'Position', [330 285 430 345]);
codigo_nl = sprintf([...
...
add_block('simulink/Continuous/Integrator', [modelName '/Int_NL'], 'Position', [460 300 490 330]);
set_param([modelName '/Int_NL'], 'InitialCondition', 'zeros(6,1)');

add_block('simulink/Signal Routing/Goto', [modelName '/Goto_X_NL'], 'Position', [530 280 580 310]);
set_param([modelName '/Goto_X_NL'], 'GotoTag', 'X_NL');
add_block('simulink/Sinks/To Workspace', [modelName '/Out_NL'], 'Position', [530 330 600 360]);
set_param([modelName '/Out_NL'], 'VariableName', 'sim_NL', 'SaveFormat', 'Array');

% Conexiones por PortHandles: puerto 1=X, puerto 2=U, puerto 3=Td
pH_Fun = get_param([modelName '/Planta_NoLineal'], 'PortHandles');
pH_Int = get_param([modelName '/Int_NL'],          'PortHandles');
pH_Sat = get_param([modelName '/Sat_NL'],          'PortHandles');
pH_Td  = get_param([modelName '/From_Td_NL'],      'PortHandles');
pH_XNL = get_param([modelName '/From_X_NL'],       'PortHandles');
pH_KNL = get_param([modelName '/K_NL'],            'PortHandles');
pH_GoX = get_param([modelName '/Goto_X_NL'],       'PortHandles');
pH_Out = get_param([modelName '/Out_NL'],          'PortHandles');

add_line(modelName, pH_XNL.Outport(1), pH_KNL.Inport(1), 'autorouting','on');
add_line(modelName, pH_KNL.Outport(1), pH_Sat.Inport(1), 'autorouting','on');
add_line(modelName, pH_Int.Outport(1), pH_Fun.Inport(1), 'autorouting','on'); % X  → puerto 1
add_line(modelName, pH_Sat.Outport(1), pH_Fun.Inport(2), 'autorouting','on'); % U  → puerto 2
add_line(modelName, pH_Td.Outport(1),  pH_Fun.Inport(3), 'autorouting','on'); % Td → puerto 3
add_line(modelName, pH_Fun.Outport(1), pH_Int.Inport(1), 'autorouting','on'); % dX → integrador
add_line(modelName, pH_Int.Outport(1), pH_GoX.Inport(1), 'autorouting','on');
add_line(modelName, pH_Int.Outport(1), pH_Out.Inport(1), 'autorouting','on');
%% S14 — SIMULACIÓN Y GRÁFICAS (DARK MODE)
fprintf('4. Simulando 10 segundos...\n');
set_param(modelName, 'Solver', 'ode45', 'StopTime', '10');
save_system(modelName); out = sim(modelName);

% Extracción Segura
if isstruct(out.sim_Lin), X_L = out.sim_Lin.signals.values; t = out.sim_Lin.time;
else, X_L = out.sim_Lin; t = out.tout; end
if isstruct(out.sim_NL), X_NL = out.sim_NL.signals.values;
else, X_NL = out.sim_NL; end

if ndims(X_L)  >= 3, X_L  = squeeze(X_L);  end
if ndims(X_NL) >= 3, X_NL = squeeze(X_NL); end
if size(X_L,1)  ~= length(t), X_L  = X_L.';  end
if size(X_NL,1) ~= length(t), X_NL = X_NL.'; end
